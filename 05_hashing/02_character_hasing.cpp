...
//              cout<<"frequency of "<<s[i]<<" "<<arr[int(s[i])-x]<<endl;
//              detect[s[i]-x]=true;
...
//  arr[(int(s[i]))] can also be written as arr[s[i]] , the arr[], the brackers needs an integer. 
// even if we write an character, it will be autocasted to its integer value.




// max size of arrays while creating a hash arrays were 10^6.
...
// in case of hasing using array, it was required to declare an array of size 13, in order to store an element of value 12.
...
// it takes log(n) in all the cases
// best,avg and worst
...
// a case may arise where after moduluo wirth 10, we are getting same numbers
